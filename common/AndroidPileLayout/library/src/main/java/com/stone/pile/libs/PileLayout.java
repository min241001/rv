...
public class PileLayout extends ViewGroup {
    private static final String TAG = "piiii";

    private final int mMaximumVelocity;
    private OnClickListener onClickListener;

    // 以下三个参数，可通过属性定制
    private int interval = 10; // view之间的间隔
    ...
    @Override
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {// width-
        int width = getDefaultSize(getSuggestedMinimumWidth(), widthMeasureSpec);
        int height = getDefaultSize(getSuggestedMinimumHeight(), heightMeasureSpec);
        everyWidth = (int)((getPaddingLeft() + getPaddingRight() + interval * 8)/displayCount);//(int) ((width - getPaddingLeft() - getPaddingRight() - interval * 8) / displayCount);
        everyHeight = (int) ( width * sizeRatio);
        Log.i(TAG,"------------------------->width:"+width);
        Log.i(TAG,"everyWidth:"+everyWidth);
        Log.i(TAG,"displayCount:"+displayCount);

        setMeasuredDimension(width, (int) (everyHeight * (1 + scaleStep) + getPaddingTop() + getPaddingBottom()));

        // 把每个View的初始位置坐标都计算好
        if (originX.size() == 0) {
            int position0 = width-everyWidth;
            originX.add(position0);

            int position1 = width-everyWidth-interval * 1;
            originX.add(position1);

            int position2 = width-everyWidth-interval * 2;
            originX.add(position2);

            int position3 = width-everyWidth-interval * 3;
            originX.add(position3);

            int position4 = width-everyWidth-((int) (position3 + everyWidth + interval));//* (1 + scaleStep)
            originX.add(position4);

            int position5 = width-everyWidth-(position4 + everyWidth + interval);
            originX.add(position5);

            int position6 = width-everyWidth-(position5 + everyWidth + interval);
            originX.add(position6);

            int position7 = width-everyWidth-(position6 + everyWidth + interval);
            originX.add(position7);

            int position8 = width-everyWidth-(position7 + everyWidth + interval);
            originX.add(position8);

            scrollDistanceMax = position3 - position4;
        }
    }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < num; i++) {
            ...
            int left =r+everyWidth;
            int top = (getMeasuredHeight() - everyHeight) ;
            int right = left - originX.get(i);
    ...
    private void adjustScale(View itemView) {
        float scale = 1.0f;
        for (int i = 3; i >= 0; i--) {
            int position = originX.get(i);
            int thisLeft = itemView.getLeft();
            if (thisLeft < position) {
                scale -= 0.1f;
            }
