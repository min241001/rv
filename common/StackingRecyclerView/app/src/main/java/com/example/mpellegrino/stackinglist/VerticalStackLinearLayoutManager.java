 ...
 */
public class VerticalStackLinearLayoutManager extends LinearLayoutManager {
    private static final String TAG = VerticalStackLinearLayoutManager.class.getName();
    @NonNull
    private final VerticalStackLinearConfig mStackConfig;
     ...
     */
    public VerticalStackLinearLayoutManager(Context context, @NonNull VerticalStackLinearConfig stackConfig, boolean stackFromEnd) {
    ...
    private void layoutStack(RecyclerView.Recycler recycler, RecyclerView.State state) {
       ...
       // int marginEnd = ((ViewGroup.MarginLayoutParams) childView.getLayoutParams()).getMarginEnd();
        int marginBottom = getHeight()-((ViewGroup.MarginLayoutParams) childView.getLayoutParams()).bottomMargin;
        int marginTop = ((ViewGroup.MarginLayoutParams) childView.getLayoutParams()).topMargin;
        int childWidth = childView.getMeasuredWidth();
        int childHeight = childView.getMeasuredHeight();

        int layerOneChildTop = childView.getTop();
        int layerOneChildBottom = childView.getBottom();
        ...
        int currentUnstackedHeight = (layerOneChildTop + childHeight + layerOneChildBottom) * lastVisibleItemPosition;

        int scrollY = computeVerticalScrollOffset(state);
        // stacks starts at last fully visible child start
        //@Px int stackStart = getWidth() - (marginStart + childWidth + marginEnd);
        @Px int stackBottom = getHeight() - (layerOneChildBottom + childHeight + layerOneChildTop);
...
//        Log.d(TAG, "currentUnstackedWidth = " + currentUnstackedWidth + " lastVisibleItemPosition = " + lastVisibleItemPosition + " childWidth = " + childWidth + " childHeight = " + childHeight + " stackStart = " + stackStart);
        ...
        for (int i = lastVisibleItemPosition; i < (lastVisibleItemPosition + numberLayers); i++) {
            if(i == lastVisibleItemPosition) {
                ...
                addView(childView, i);
                //measureChildWithMargins(childView, 0, getHeight()/2);
                measureChildWithMargins(childView, 0, getHeight());
            }
            ...
            currentUnstackedHeight += childView.getBottom();
           // HorizontalStackLinearConfig.StackLayer layer =
            ...
            VerticalStackLinearConfig.StackLayer layer = mStackConfig.calculateLayer(scrollY,getHeight(),currentUnstackedHeight
                            , marginBottom + childHeight + marginTop);
            Log.d(TAG, "calculateLayer position = " + i + " scrollY = " + scrollY + " parentWidth = " + getWidth() + " childStart = " + currentUnstackedHeight + " childWidth = " + (marginBottom + childWidth + marginTop));
            ...
            if(layer != null){
                int childBottom= stackBottom + layer.getOffsetY();
                ...
                layoutDecoratedWithMargins(childView, childBottom, childTop, childBottom, childTop + childHeight);
                //TODO: why not set entire childView alpha
                childView.findViewById(R.id.opaque_part).setAlpha(layer.getAlpha());
                //getDecoratedBottom(childView);
            ...
            currentUnstackedHeight += childHeight + marginTop;
        }
    }
