...
/**
 * Used to configure the layer creation for a {@link VerticalStackLinearLayoutManager}.
 */
public class VerticalStackLinearConfig {
     ...
     */
    public VerticalStackLinearConfig(int numberLayers, float startAlpha, int offsetX, int offsetY) {
        if(numberLayers <= 0){
    ...
    public StackLayer calculateLayer(int parentStart,
        ...
        int offScreen = childStart + childHeight - parentHeight - parentStart;
        if (offScreen <= 0) {
            //no part of child would be offscreen
            return null;
        }

        float layer = (float)offScreen / (float)childHeight;
