...
public class TimeMilliSecondView extends View {

    private Context mContext;
    private Paint mPaint;
    private float mMillSecondDegree;
    ...
    public TimeMilliSecondView(Context context) {
        super(context);
        this.mContext = context;
        initPainter();
    }

    public TimeMilliSecondView(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.mContext = context;
        init(context, attrs);
        initPainter();
...
//        canvas.translate(0, borderWidth / 2 + maxScaleLength + textHeight * 3f / 4 - getWidth() / 3);
        canvas.translate(0, 18 - getWidth() / 2);
        canvas.rotate(-degree);
...
//        canvas.translate(0, getWidth() / 3 - (borderWidth / 2 + maxScaleLength + textHeight * 3f / 4));
        canvas.translate(0, getWidth() / 2 - 18);
        canvas.rotate(-degree);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawCircle(getWidth() / 2, getHeight() / 2, getWidth() / 2-borderWidth/2, mPaint);
        //圆背景
        ...
        for (int i = 0; i < 60; i++) {
            if (i % 6 == 0) {//长刻度
                ...
                canvas.drawLine(getWidth() / 2, getHeight() / 2 - (getWidth() / 2 - borderWidth / 2),
                        getWidth() / 2, getHeight() / 2 - (getWidth() / 2 - borderWidth / 2) + maxScaleLength, mPaint);
            } else {//短刻度
                mPaint.setColor(minScaleColor);
                canvas.drawLine(getWidth() / 2, getHeight() / 2 - (getWidth() / 2 - borderWidth / 2),
                        getWidth() / 2, getHeight() / 2 - (getWidth() / 2 - borderWidth / 2) + minScaleLength, mPaint);
            }
        ...
        if (isDrawText) {
            ...
            for (int i = 0; i < 10; i++) {
                if (i == 0) {
                    drawNum(canvas, i * 36, 0 + "", mPaint);
                } else {
                    drawNum(canvas, i * 36, i + "", mPaint);
                }
            }
            canvas.restore();
        }
        ...
        //毫秒
        canvas.save();
        mPaint.setColor(Color.parseColor("#ff5603"));
        mPaint.setColor(secondPointerColor);
        mPaint.setStrokeWidth(2);
        canvas.rotate(mMillSecondDegree, getWidth() / 2, getHeight() / 2);
...
//            mHourDegree = min * 0.5f + hour * 30f;
    ...
    public void setTotalMillSecond(long mTotalMillSecond) {

        int hour = (int) mTotalMillSecond / 1000 / 3600;
        int min = (int) (mTotalMillSecond - hour * 3600 * 1000) / 1000 / 60;
        int second = (int) (mTotalMillSecond - hour * 3600 * 1000 - min * 60 * 1000)/1000;
        int milliSecond = (int) (mTotalMillSecond - hour * 3600 * 1000 - min * 60 * 1000 - second * 1000);
        if (mMillSecondDegree == 360) {
            mMillSecondDegree = 0;
        }
        mMillSecondDegree = milliSecond / 16 * 6f;
