...
public class TimeSecondsView extends View {
    ...
    public TimeSecondsView(Context context) {
        super(context);
        this.mContext = context;
        initPainter();
    }

    public TimeSecondsView(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.mContext = context;
        init(context, attrs);
        initPainter();
...
//        mPaint.setColor(Color.parseColor("#ff5603"));
        ...
        canvas.drawLine(getWidth() / 2, getHeight() / 2, getWidth() / 2,
                getHeight() / 2 - getHeight()/2, mPaint); //stopY  后一个getHeight()/2表示指针长度
        canvas.restore();
