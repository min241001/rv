...
import com.baehug.callui.phone.service.PhoneCallService
import com.baehug.callui.phone.utils.GlobalActivityLifecycleMonitor
import com.baehug.callui.phone.view.PhoneCallActivity
...
class PhoneCallManager private constructor() {
    ...
    init {
        audioManager = App.getInstance().getSystemService(Context.AUDIO_SERVICE) as? AudioManager
    }
    ...
    fun addCall(call: Call) {
        ...
        Log.i(TAG, "addCall: call $call isForeground $isForeground")
        PhoneCallActivity.actionStart(App.getInstance(), callId, isForeground)
    }
    ...
    @RequiresApi(Build.VERSION_CODES.M)
    fun playNumberTone1(callId: String?, digit: Char): Boolean {
        val call = getCallById(callId)
        if (call != null) {
            Log.i(TAG, "playNumberTone1: callId $callId digit $digit")
    ...
    @RequiresApi(Build.VERSION_CODES.M)
    fun stopDtmfTone(callId: String?): Boolean {
        val call = getCallById(callId)
        if (call != null) {
            Log.i(TAG, "stopDtmfTone: callId $callId")
            call.stopDtmfTone()
            return true
        }
        return false
    ...
    fun getSlotIcon(callId: String?) =
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                getCallById(callId)?.let {
                    val telecomsManager = App.getInstance().getSystemService(Context.TELECOM_SERVICE) as? TelecomManager
                    telecomsManager?.getPhoneAccount(it.details.accountHandle)?.icon?.loadDrawable(App.getInstance())
                }
            } else null
    ...
    fun isEnableToChangeDefaultPhoneCallApp() =
            buildSetDefaultPhoneCallAppIntent()?.resolveActivity(App.getInstance().packageManager) != null
    ...
    fun isDefaultPhoneCallApp(): Boolean {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            val manger = App.getInstance().getSystemService(Context.TELECOM_SERVICE) as? TelecomManager
            if (manger != null && manger.defaultDialerPackage != null) {
                return manger.defaultDialerPackage == App.getInstance().packageName
            }
        }
        return false
    }
    ...
    fun buildSetDefaultPhoneCallAppIntent(): Intent? {
        return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M)
            Intent().apply {
                ...
                putExtra(TelecomManager.EXTRA_CHANGE_DEFAULT_DIALER_PACKAGE_NAME, App.getInstance().packageName)
            } else null
    }
     ...
     * 添加一个通话
     */
    fun addOneMoreCall(context: Context? = App.getInstance()): Boolean {
        if (!canAddCall()) {
            return false
        }
