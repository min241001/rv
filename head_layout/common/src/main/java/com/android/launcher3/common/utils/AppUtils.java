...
public class AppUtils {
    ...
    //获取当前设备的mac地址
    //从Android 6.0开始，为了保护用户的隐私，无法直接访问设备的MAC地址。
    //若需要唯一标识设备，可以考虑使用Android ID或者广告ID等替代。
    ...
    public static boolean isThemeDial(){
        return AppLocalData.getInstance().getFaceDefaultId() > 999;
    }

    /**
     * 获取蜂窝桌面图标层数，蜂窝图标为正6边形。1 + 6 + 12 + 18 + 24 + 30 + 36
     * 如下图：
     *     2     3
     *   4    1    5
     *     6     7
     ...
     * @return 层数
     */
    public static int getAppLayer(Context context) {
        if (context == null) {
            return 6;
        }
        AppMode appMode = new AppMode(context);
        List<AppBean> appBeans = appMode.loadDefaultData();
        int size = appBeans.size();
        LogUtil.i("getAppLayer: " + size, LogUtil.TYPE_DEBUG);
        if (size < 7) {
            return 2;
        } else if (size <= 19) {
            return 3;
