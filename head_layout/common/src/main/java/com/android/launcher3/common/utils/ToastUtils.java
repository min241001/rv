...
public class ToastUtils {
    ...
    public static void show(int text) {
        ...
        if (Looper.myLooper() == Looper.getMainLooper()) {
            ...
            if (mToast == null){
                mToast =  Toast.makeText(CommonApp.getInstance(), text, Toast.LENGTH_SHORT);
            }else {
                mToast.setText(text);
            ...
            handler.post(() -> {
                ...
                if (mToast == null){
                    mToast = Toast.makeText(CommonApp.getInstance(), finalText, Toast.LENGTH_SHORT);
                }else {
    ...
    public static void show(String text) {
        if (text.contains("Unable to resolve host")){
            text = CommonApp.getInstance().getString(R.string.no_network);
        }
        //判断当前是主线程还是子线程
        if (Looper.myLooper() == Looper.getMainLooper()) {
            //当前looper是否等于主线程looper, 如果是, 说明当前是在主线程
    ...
    public static void show4Debug(final String text) {
        if (true) {
            return;
        }
        //判断当前是主线程还是子线程
        if (Looper.myLooper() == Looper.getMainLooper()) {
            //当前looper是否等于主线程looper, 如果是, 说明当前是在主线程
            Toast.makeText(CommonApp.getInstance(), text, Toast.LENGTH_SHORT).show();
