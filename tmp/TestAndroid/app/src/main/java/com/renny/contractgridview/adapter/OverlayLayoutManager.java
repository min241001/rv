...
public class OverlayLayoutManager extends RecyclerView.LayoutManager {
    ...
    private void addAndLayoutViewVertical(RecyclerView.Recycler recycler, RecyclerView.State state, int offset) {
        ...
        Log.i(Constants.sTAG, "--------------------addAndLayoutViewVertical");
    ...
    private void adjustScale(View itemView, float scale, float alpha, float slowTimes, int ChildCount, int i, boolean flag) {
        Log.i(TAG, "----------->i:" + i);
        Log.i(TAG, "-------------->getChildItemCount:" + ChildCount);

        scale = 1.0f;
        alpha = 1.0f;
       // scale -= (slowTimes - ChildCount + 2) * 0.15f;
       // alpha -= (slowTimes - ChildCount + 2) * 0.15f;
        scale += (ChildCount - (slowTimes+2)) * 0.15f;
        alpha += (ChildCount - (slowTimes+2)) * 0.25f;
        if (scale > 0.8f) {
            //scale = 1.0f;
        }
        ...
        //scale -= i*0.15f;
        //alpha -= i*0.15f;
        Log.i(Constants.sTAG, "scale:" + scale);
        Log.i(Constants.sTAG, "alpha:" + alpha);
        if (scale < 0.0f) {
            scale = 0.0f;
        }
        if(scale>1.0f){
