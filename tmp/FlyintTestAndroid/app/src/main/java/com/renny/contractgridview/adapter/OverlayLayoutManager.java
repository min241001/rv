...
import com.renny.contractgridview.bean.ViewInfo;
...
public class OverlayLayoutManager extends RecyclerView.LayoutManager {
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        CalcChildView(recycler, state, index2, false);
    }
    ...
    private void CalcChildView(RecyclerView.Recycler recycler, RecyclerView.State state, int index, boolean flag) {
        ...
        if (orientation == OrientationHelper.VERTICAL) {
            calculateChildrenSiteVertical2(recycler, state, index, flag);
            addAndLayoutViewVertical(recycler, state, verticalScrollOffset);
        } else {
    ...
    private void calculateChildrenSiteVertical2(RecyclerView.Recycler recycler, RecyclerView.State state, int index, boolean scrollFlag) {
        boolean flag = false;
    ...
    private void addAndLayoutViewVertical(RecyclerView.Recycler recycler, RecyclerView.State state, int offset) {
        ...
        for (int i = itemCount - 1; i >= 0; i--) {
            ...
            //LogUtil.i(Constants.tm, "add view i:" + i);
            ...
            int topOffset = (i - 2) * height+height0 - offset;
            if (i < 1) {
                bottomOffset = height0 - offset;
                topOffset = 0;
            }
    ...
    private int getVerticalSpace(int height) {
        // 计算RecyclerView的可用高度，除去上下Padding值
        return getHeight() - getPaddingBottom() - getPaddingTop()+120;
    }
