...
import com.baehug.callui.phone.service.NotificationService
    ...
    fun answer() {
        when {
            Build.VERSION.SDK_INT >= Build.VERSION_CODES.P -> {
                val telecomManager = App.getInstance().getSystemService(Context.TELECOM_SERVICE) as TelecomManager
                if (ActivityCompat.checkSelfPermission(App.getInstance(), Manifest.permission.ANSWER_PHONE_CALLS) != PackageManager.PERMISSION_GRANTED) {
                    return
                }
    ...
    private fun finalAnswer() {
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                val mediaSessionManager = App.getInstance().getSystemService("media_session") as MediaSessionManager
                val activeSessions = mediaSessionManager.getActiveSessions(ComponentName(App.getInstance(), NotificationService::class.java)) as List<MediaController>
    ...
    private fun answerPhoneAidl() {
        try {
            ...
            if (Build.VERSION.SDK_INT >= 19) {
                @SuppressLint("WrongConstant") val audioManager = App.getInstance().getSystemService("audio") as AudioManager
            ...
            App.getInstance().sendOrderedBroadcast(intent, "android.permission.CALL_PRIVILEGED")
            ...
            App.getInstance().sendOrderedBroadcast(intent2, "android.permission.CALL_PRIVILEGED")
        }
    }
